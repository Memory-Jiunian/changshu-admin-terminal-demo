...
export function StudentProfileDetailContent({ detail, notice, onViewCaseRecord, onViewWarning }: { detail: StudentProfileDetail; notice?: string; onViewCaseRecord: (warningId: string) => void; onViewWarning: (warningId: string) => void }) {
  return (
    <div className="pb-6">
      {notice ? (
        <div className="mx-6 mt-5 rounded-md border border-[var(--warning-200)] bg-[var(--warning-50)] p-3 text-sm text-[var(--warning-700)]">{notice}</div>
      ) : null}
      {detail.dataIssues.length > 0 ? (
        <div className="mx-6 mt-5 rounded-md border border-[var(--warning-200)] bg-[var(--warning-50)] p-3 text-sm text-[var(--warning-700)]">
