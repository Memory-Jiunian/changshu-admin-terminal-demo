...
export function StudentProfilePrintableReport({ report }: { report: StudentProfileExportViewModel }) {
  return <article className="print-report" aria-hidden="true">
    ...
    {report.cases.map((item) => <section className="print-case" key={item.summary.warningId}>
      ...
      <h3>干预预约与记录</h3>{item.interventionHistory.rounds.length ? item.interventionHistory.rounds.map(({ appointment, result }, index) => <div key={appointment.id}><p><strong>干预轮次 {item.interventionHistory.rounds.length - index}</strong> · 计划 {appointment.plannedAt} · {appointment.location} · {appointment.responsibleTeacher} · {appointment.status}；陪同班主任：{appointment.escortTeacher || "-"}；创建：{appointment.createdAt} · {appointment.createdBy}；通知计划：{getInterventionNotificationPlan(appointment).map((plan) => `${plan.label} ${plan.expectedAt}`).join("；")}</p>{result ? <p>结果：{result.occurredAt} · {result.authorName} · {result.method}；情况摘要：{result.summary}；本次判断：{result.judgment}；下一步计划：{result.followUpPlan}</p> : <p>尚未记录本次干预结果</p>}</div>) : <p>暂无</p>}{item.interventionHistory.unlinkedRecords.length ? <div><strong>未关联历史记录</strong>{item.interventionHistory.unlinkedRecords.map((record) => <p key={record.id}>{record.occurredAt} · {record.authorName} · {record.method}；情况摘要：{record.summary}；本次判断：{record.judgment}；下一步计划：{record.followUpPlan}</p>)}</div> : null}
