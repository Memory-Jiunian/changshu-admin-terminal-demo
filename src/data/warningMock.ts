import type {
  ...
  WarningItem,
  WarningReferralFollowUpRecord,
  WarningReferralRecord,
} from "@/types/warning";

type LegacyReferralFollowUpRecord = Omit<WarningReferralFollowUpRecord, "conclusion"> &
  Partial<Pick<WarningReferralFollowUpRecord, "conclusion">>;

type LegacyReferralRecord = Omit<WarningReferralRecord, "followUpRecords"> & {
  followUpRecords?: LegacyReferralFollowUpRecord[];
};
...
const warningMockSeeds: WarningMockSeed[] = [
  ...
  },
  {
    ...
    feedbackRequestNote: "请持续观察学生到校、睡眠和课堂参与情况。",
    responsibleTeacher: "陈老师",
  ...
  },
  {
    ...
    confirmedRiskLevel: "critical",
    currentStatus: "pending_retest",
    latestActivity: "转介后复测情况待确认",
    activityTime: "2026-07-08 09:10",
    feedbackStatus: "pending_feedback",
    responsibleTeacher: "陈老师",
    ...
    retestRecords: [{
      id: "RR-006-1",
      arrangedAt: "2026-07-05 17:30",
      plannedAt: "2026-07-07 15:00",
      scaleIds: ["phq-9"],
      scaleNames: ["PHQ-9 抑郁症筛查量表"],
      note: "转介跟进后安排复测，当前尚未确认完成情况。",
    }],
    referralRecords: [
      {
        id: "RF-006-1",
...
const normalizedWarningMockData: WarningItem[] = warningMockSeeds.map((warning, index) => {
  ...
  if (warning.id === "WRN-20260707-004") {
    interventionAppointments.unshift({
      id: "IA-WRN-20260707-004-NEXT",
      plannedAt: "2026-07-08 16:00",
      location: "心理咨询室",
      responsibleTeacher: warning.responsibleTeacher,
      escortTeacher: headTeacherName,
      status: "planned",
      createdAt: "2026-07-07 17:15",
      createdBy: warning.responsibleTeacher,
      notificationOffsetsMinutes: [1440, 120],
    });
  }
  const retestRecords = warning.retestRecords.map((record, recordIndex) => ({
    ...record,
    assessmentRecordId: record.completedAt
      ? deepAssessmentRecords[Math.min(recordIndex, deepAssessmentRecords.length - 1)]?.id
      : undefined,
    appointmentStatus: record.completedAt ? "completed" as const : "planned" as const,
  }));
  ...
  const referralRecords = (warning.referralRecords ?? []).map((record) => ({
    ...
    followUpRecords: record.followUpRecords?.length
      ? [...record.followUpRecords]
          .map((item) => ({ ...item, conclusion: item.conclusion || "继续跟进转介进展。" }))
...
const interventionStatusPendingDemo: WarningItem = {
  ...normalizedWarningMockData[0],
  id: "WRN-20260708-013",
  studentId: "STU-0013",
  studentName: "顾晨曦",
  gradeClass: "初二（6）班",
  sourceType: "teacher_report",
  evidenceTypes: ["teacher_report", "deep_assessment"],
  suggestedRiskLevel: "high",
  confirmedRiskLevel: "high",
  currentStatus: "in_intervention",
  latestActivity: "干预预约时间已过，等待确认到场情况",
  activityTime: "2026-07-08 09:20",
  feedbackStatus: "pending_feedback",
  responsibleTeacher: "陈老师",
  headTeacherName: "孙老师",
  headTeacherPhone: "138****1313",
  assessmentSummary: "补充评估完成，心理老师已确认正式预警并安排首次干预。",
  aiSummary: "当前事项无新增 AI 倾诉线索。",
  deepAssessmentRecords: [],
  aiConversationRecords: [],
  teacherFeedbackSummary: "尚未创建班主任反馈任务。",
  feedbackRecords: [],
  feedbackRequests: [{
    id: "FQ-WRN-20260708-013-1",
    requestedAt: "2026-07-06 15:40",
    requestedBy: "陈老师",
    requestNote: "请持续观察学生到校和预约前情绪变化。",
    deadline: "2026-07-09 17:00",
    status: "pending",
  }],
  hasUnreadFeedback: false,
  interventionRecords: [],
  interventionAppointments: [{
    id: "IA-WRN-20260708-013-1",
    plannedAt: "2026-07-07 15:30",
    location: "心理咨询室",
    responsibleTeacher: "陈老师",
    escortTeacher: "孙老师",
    status: "planned",
  ...
  timeline: [{
    ...
    title: "预约干预",
    operator: "陈老师",
    occurredAt: "2026-07-06 16:00",
    description: "计划于 2026-07-07 15:30 在心理咨询室开展干预，当前等待心理老师确认到场结果。",
  }, {
    id: "TL-013-1",
    title: "确认正式预警",
    operator: "陈老师",
    occurredAt: "2026-07-06 15:40",
    description: "心理老师确认高风险并进入正式预警。",
  }],
  isActive: true,
  disposition: "active",
  endedAt: undefined,
  endReason: undefined,
  feedbackDeadline: "2026-07-09 17:00",
  feedbackRequestNote: "请持续观察学生到校和预约前情绪变化。",
};

const observingFeedbackReceivedDemo: WarningItem = {
  ...normalizedWarningMockData[1],
  id: "WRN-20260708-014",
  studentId: "STU-0014",
  studentName: "沈知夏",
  nextReviewAt: "2026-07-10 10:00",
  activityTime: "2026-07-08 10:20",
  latestActivity: "班主任已提交本轮观察反馈",
  feedbackStatus: "feedback_received",
  feedbackDeadline: "2026-07-08 10:00",
  feedbackRequestNote: "请反馈学生近期到校和课堂参与情况。",
  feedbackRequests: [{
    id: "FQ-WRN-20260708-014-1",
    requestedAt: "2026-07-07 09:00",
    requestedBy: "陈老师",
    requestNote: "请反馈学生近期到校和课堂参与情况。",
    deadline: "2026-07-08 10:00",
    status: "completed",
  }],
  feedbackRecords: [{
    id: "FB-WRN-20260708-014-1",
    requestId: "FQ-WRN-20260708-014-1",
    authorRole: "班主任",
    authorName: "王老师",
    content: "学生本周按时到校，课堂参与较前一周有所增加。",
    submittedAt: "2026-07-08 10:20",
    psychologistReadAt: "2026-07-08 10:30",
  }],
  teacherFeedbackSummary: "学生本周按时到校，课堂参与有所增加。",
  hasUnreadFeedback: false,
};

const observingFeedbackOverdueDemo: WarningItem = {
  ...normalizedWarningMockData[1],
  id: "WRN-20260708-015",
  studentId: "STU-0015",
  studentName: "陆安然",
  nextReviewAt: "2026-07-10 15:00",
  activityTime: "2026-07-07 09:00",
  latestActivity: "观察反馈任务已超过截止时间",
  feedbackStatus: "feedback_overdue",
  feedbackDeadline: "2026-07-07 17:00",
  feedbackRequestNote: "请反馈学生近期请假和同伴互动情况。",
  feedbackRequests: [{
    id: "FQ-WRN-20260708-015-1",
    requestedAt: "2026-07-06 09:00",
    requestedBy: "陈老师",
    requestNote: "请反馈学生近期请假和同伴互动情况。",
    deadline: "2026-07-07 17:00",
    status: "overdue",
  }],
  feedbackRecords: [],
  teacherFeedbackSummary: "已请求，暂未收到反馈。",
  hasUnreadFeedback: false,
};

const retestTodayDemo: WarningItem = {
  ...normalizedWarningMockData[8],
  id: "WRN-20260708-016",
  studentId: "STU-0016",
  studentName: "唐语晴",
  currentStatus: "pending_retest",
  latestActivity: "已安排今日复测",
  activityTime: "2026-07-08 09:00",
  retestRecords: [{
    id: "RR-WRN-20260708-016-1",
    arrangedAt: "2026-07-08 09:00",
    plannedAt: "2026-07-08 17:00",
    scaleIds: ["phq-9"],
    scaleNames: ["PHQ-9 抑郁症筛查量表"],
    note: "今日按计划完成阶段复测。",
    appointmentStatus: "planned",
  }],
  timeline: normalizedWarningMockData[8].timeline.filter(
    (item) => item.title !== "学生完成复测",
  ),
};
