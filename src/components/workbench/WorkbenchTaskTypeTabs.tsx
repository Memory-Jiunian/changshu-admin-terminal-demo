...
export function WorkbenchTaskTypeTabs({ value, counts, onChange }: { value: WorkbenchTaskFilter; counts: Record<WorkbenchTaskType, number>; onChange: (value: WorkbenchTaskFilter) => void }) {
  return <Tabs onValueChange={(next) => onChange(next as WorkbenchTaskFilter)} value={value}>
    <TabsList className="scrollbar-hidden h-auto w-full flex-nowrap justify-start gap-1 overflow-x-auto rounded-none border-b border-neutral-200 bg-white p-3">
      <TabsTrigger className="shrink-0" value="all">全部</TabsTrigger>
      {types.map((type) => <TabsTrigger className="shrink-0" key={type} value={type}>{workbenchTaskLabels[type]} {counts[type]}</TabsTrigger>)}
    </TabsList>
  </Tabs>;
}
