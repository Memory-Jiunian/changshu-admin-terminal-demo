...
const riskClass = { low: "bg-emerald-50 text-emerald-700", medium: "bg-neutral-100 text-neutral-700", high: "bg-neutral-800 text-white", critical: "bg-red-50 text-red-800 border-red-200" } as const;
...
export function WorkbenchTaskItem({ task, selected, onOpen }: { task: WorkbenchTask; selected: boolean; onOpen: (task: WorkbenchTask) => void }) {
  return <article className={cn("grid gap-4 border-b border-neutral-100 px-5 py-4 last:border-0 lg:grid-cols-[minmax(180px,0.8fr)_minmax(260px,1.5fr)_auto]", selected && "bg-neutral-50")}>
    <div><div className="font-semibold text-neutral-950">{task.studentName}</div><div className="mt-1 text-sm text-neutral-500">{task.gradeClass} · {task.responsibleTeacher}</div><Badge className={cn("mt-2", riskClass[task.riskLevel])} variant="outline">{riskLevelLabels[task.riskLevel]}</Badge></div>
